...
A central concept in our study is the *emittance*, a measure of volume in phase space. This section defines the four-dimensional emittance and its relationship to the two-dimensional *intrinsic* and *projected* emittances. Later, we'll use the intrinsic emittances as a figure of merit for our experiments.

Let $\mathbf{x} = (x, x', y, y')$ be the phase space coordinate vector, where $x$-$y$ and $x'$-$y'$ are the transverse positions and velocities, respectively. Let $\mathbf{\Sigma}$ be the covariance matrix:
...
We will also define the *projected* emittances. The four-dimensional ellipsoid projects to a two-dimensional ellipse on the $x$-$x'$ or $y$-$y'$ plane. The projected emittances $\varepsilon_{x, y}$ are the areas enclosed by these ellipses. Writing the covariance matrix as
...
The projected emittances are conserved under uncoupled linear symplectic transformations, i.e., transformations that act in only one plane. 

The most important point for our analysis is that *the intrinsic emittances are fundamental, not the projected emittances*. The intrinsic emittances constrain the projected emittances, which must satisfy

$$
\varepsilon_x \varepsilon_y \ge \varepsilon_1 \varepsilon_2 .
$${#eq-emitt-relationship}

The equality in @eq-emitt-relationship occurs when $\mathbf{\Sigma}_{xy} = 0$, i.e., when there are no cross-plane correlations in the beam. We can think of the intrinsic emittances as the projected emittances after a symplectic diagonalization of the covariance matrix.
...
Projected rms ellipses before and after normalization (symplectic diagonalization of the covariance matrix).
:::


## Vortex distributions {#sec-vortex}

A *vortex* distribution is defined to have a large ratio between its intrinsic emittances: $\varepsilon_1 \gg \varepsilon_2$. Such distributions have large *vorticity*, or curl of the velocity field @groening_2021_vorticity. Examples are shown in @fig-vortex.

![Examples of vortex distributions. Each distribution has a large intrinsic emittance ratio: $\varepsilon_1 \gg \varepsilon_2$, corresponding to a high-curl velocity field.](images/vortex.png){#fig-vortex fig-align=center width=400px}

There are a few potential applications of vortex distributions which I'll highlight here.


### Vortex distributions in hadron colliders

First, vortex beams could be used to boost the luminosity of hadron collider @burov_2013_circular. To maximize the luminosity (the number of collisions per unit time per unit area), we want to maximize the particle density at the collision point, for example by flattening the $x$-$y$ distribution. Unfortunately, space charge places fundamental limits on the beam density during the low-energy accumulation stages. But we can play a trick: we can accumulate a round vortex beam, accelerate it to high energy, and then remove its angular momentum using a series of skew quadrupoles. The skew quadrupoles simply map the projected emittances to the intrinsic emittances, as in @fig-cov. Since the initial beam has a large intrinsic emittance ratio ($\varepsilon_1 \gg \varepsilon_2$), the final beam has a large projected emittance ratio ($\varepsilon_x \gg \varepsilon_y$). This is the so-called round-to-flat (RTF) transformation @kim_2003_round shown in @fig-roundflat. Since inter-particle forces are negligble at the higher energy, this scheme essentially bipasses the space charge limit on the beam density.
...
To review: I've listed several applications of vortex beam distributions: the production of flat beams in hadron colliders, the suppression of space charge structure resonances, and the reduction of space-charge-driven tune spread. We now introduce *eigenpainting*, a method to accumulate a vortex distribution via phase space painting in an accumulator ring. 

I'm going to start by describing two conventional painting methods known as *correlated* and *anti-correlated* painting. [...]
