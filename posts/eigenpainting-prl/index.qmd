...
Our study focuses on two related questions. First, what is the ideal beam distribution? After coving some relevant background in @sec-emitt, I'll argue in @sec-vortex that *vortex distributions* are attractive options for several different applications of high-intensity beams. Second, suppose we've identified a target distribution: can we paint it? I'll address this question in @sec-eigenpainting, which introduces *eigenpainting* as a method to paint vortex distributions in the presence of space charge. Finally, in @sec-exp, I'll explain our attempt to implement this method in the [Spallation Neutron Source (SNS)]() accumulator ring and explain our results.


## Background: intrinsic and projected emittances {#sec-emitt}

A central concept in our study is the *emittance*, a measure of volume in phase space. This section defines the four-dimensional emittance and its relationship to the two-dimensional *intrinsic* and *projected* emittances.

Let $\mathbf{x} = (x, x', y, y')$ be the phase space coordinate vector, where $x$-$y$ and $x'$-$y'$ are the positions and velocities, respectively. Let $\mathbf{\Sigma}$ be the covariance matrix of the four-dimensional phase space distribution:
...
The four-dimensional emittance $\varepsilon_{4D}$ is the square root of the determinant of the covariance matrix:

$$
\varepsilon_{4D} = \sqrt{\det{\mathbf{\Sigma}}}.
$${#eq-4d-emitt}

It characterizes volume of an ellipsoid by the expression $\mathbf{x}^T \mathbf{\Sigma}^{-1} \mathbf{x} = 1$ and is conserved under linear symplectic transformations. We can also write the four-dimensional emittance as the product $\varepsilon_{4D} = \varepsilon_{1}\varepsilon_{2}$, where $\varepsilon_{1, 2}$ are the individually conserved *intrinsic* emittances. The intrinsic emittances are found by symplectic diagonalization of the covariance matrix, i.e., there is a symplectic matrix $\mathbf{V}$ such that
...
In addition to the intrinsic emittances, we need to define the more common *projected* emittances. To do so, it is convenient to split the covariance matrix into four blocks:
