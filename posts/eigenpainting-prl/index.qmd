...
Eigenpainting is a method to paint *stationary* four-dimensional phase space distributions, even in the presence of space charge. The method is derived from the eigenvector analysis of coupled motion, which I'll summarize in @sec-method. [...]
...
Thus, the amplitudes $J_{1, 2}$ are invariants of the motion.

The real component of eigenvector $\mathbf{v}_k$ traces a two-dimensional ellipse of area $2 J_k$ at frequency $\mu / 2\pi$. We can see this in @fig-track-ellipse, which plots the eigenvector paths on top of the tracking data. If there is no coupling in the lattice (block-diagonal transfer matrix), the first ellipse will align with the $x$-$x'$ axis and the second with the $y$-$y'$ axis; otherwise the ellipses can have different orientations.
...
The evolution of the normalized coordinates $\mathbf{u} = \mathbf{V}^{-1} \mathbf{x}$ is [...]
...
![](images/track_norm.png){#fig-track-norm fig-align=left width=450px}


### Single-mode injection

Suppose we inject particles at a fixed transverse position and momentum. The injected particles will advance in phase at a constant amplitude in each mode, eventually filling the ellipse with uniform density. The distribution therefore depends only on the injection amplitudes $J_{1, 2}, not the phases; our paint brush is an ellipse, rather than a point, in the four-dimensional phase space. By varying the amplitudes as a function of time, we can generate arbitrary elliptically symmetric distributions $f(J_1)$ and $f(J_2)$. The full distribution $f(J_1, J_2)$ depends on the path taken through $J_1$-$J_2$ space.

We define *eigenpainting* as injection into only one mode. In other words, we move only along $J_1$ or $J_2$.

![UPDATE](images/paint_diag.png){#fig-paint-diag width=200}

We focus specifically on the choice 
...
where $\tilde{J}_1$ is a maximum value of $J_1$. In normalized coordinates, the distribution will look like this:

![Eigenpainting in normalized coordinates.](images/paint_eig_n.gif){#fig-paint-eig-n width=500px fig-align=left}

The actual distribution will depend on $\mathbf{V}$, which encodes the structure of the accelerator lattice. @fig-paint-eig shows the result of painting into the SNS ring.

![Eigenpainting in unnormalized coordinates.](images/paint_eig.gif){#fig-paint-eig width=500px fig-align=left}

Notice that the accumulated distribution is stationary with respect to turn number. The only change is a global scaling factor as particles are added to the edge of the beam ellipse.

Incredibly, this scheme works even if we include space charge forces in the dynamics.

[Explain features of distribution.]


## Vortex distributions for space charge mitigation {#sec-vortex}
